...
type Matcher struct {
	...
	// to the root, so the table only pays for the bytes actually used. Input
	// bytes outside ASCII are folded onto byte 0, as they always have been.
	alphabet [256]uint16
...
func (m *Matcher) setAlphabet(present *[maxchar]bool) {
	...
	// Non-ASCII input bytes are treated as byte 0, which is what the
	// pointer-based implementation did by clamping the child index.
	for b := maxchar; b < 256; b++ {

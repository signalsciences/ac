...
func TestAC(t *testing.T) {
	for _, tt := range cases {
		...
		// Can't use reflect.DeepEqual here because the matches are from the
		// input text and can vary in casing. Instead loop over expected vs.
		// matches and do a case insensitive compare.
		found := 0
		for _, match := range matches {
			for _, expected := range tt.matches {
				if strings.EqualFold(match, expected) {
					found++
				}
			}
		}
		if found != len(tt.matches) {
			t.Errorf("%s: FindAllString want %v, got %v", tt.name, tt.matches, matches)
		} else {
			t.Logf("Found strings: %v", matches)
		}

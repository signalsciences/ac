...
const maxchar = int(unicode.MaxASCII + 1)
...
// addChild: adds a child node to parent if it doesn't exist already.
func (m *Matcher) addChild(parent *node, c int, b []byte) *node {
	nn := parent.child[c]
	if nn == nil {
		nn = m.getFreeNode()
		parent.child[c] = nn
...
func (m *Matcher) buildTrie(dictionary [][]byte) error {
	...
	for _, blice := range dictionary {
		blice = bytes.ToLower(blice)
		n := m.root
		for i, b := range blice {
			idx := int(b)
			if idx > unicode.MaxASCII {
				return ErrNotASCII
			}

			n = m.addChild(n, idx, blice[0:i+1])
		}
...
func (m *Matcher) buildTrieString(dictionary []string) error {
	...
	for _, blice := range dictionary {
		...
		for i := 0; i < len(blice); i++ {
			...
			n = m.addChild(n, idx, []byte(blice[0:i+1]))
		}
...
func (m *Matcher) FindAllString(in string) []string {
	...
	for idx := 0; idx < slen; idx++ {
		c := int(unicode.ToLower(rune(in[idx])))
		if c >= maxchar {
			c = 0
		}
		if !n.root && n.child[c] == nil {
